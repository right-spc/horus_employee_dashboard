...
- **Schema: ✅ applied (`20260917000000_voice_schema.sql`)** — new `voice` domain schema (one-domain-per-schema pattern): `voice.phone_numbers` (1/org, UNIQUE org+number), `voice.configs` (per-org receptionist config, server CHECKs for transfer/forward numbers), `voice.calls` (per-call records; **no recording_url, no core bridge view** — privacy rule structural: PostgREST can't reach raw call rows). `core.phone_numbers` + `core.voice_configs` security-invoker bridge views (verified 200 via PostgREST). `conversations.channel` CHECK now includes `'voice'`. RLS org-isolation on all 3. Still TODO in 1a: `core.org_services` voice row per org; verify `crm.contact_aliases` phone aliases.
