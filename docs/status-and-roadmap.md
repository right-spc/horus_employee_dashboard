...
- **Phase 0 spike: ✅ COMPLETE (2026-09-13, ~$0.80)** — real PSTN calls to +1 855 585 5518 via test assistant. **K2.6 latency owner-approved on a full 41K production prompt** (no haiku A/B needed); KB-in-instructions confirmed (buried facts answered accurately); dual-channel recording + transcript API ✓; cost ≈$0.076/min blended. Gotchas captured in plan: API assistants have NO default hangup tool (must attach), Ultra heteronym stumbles (never use "live" in prompts; per-org pronunciation dictionary in Phase 1), Telnyx GET/PATCH shape flakiness (never PATCH unchecked instructions — 2 wipe-and-rebuilds). **Voice shortlist curated: Rachel, Amber, Reed, Carson, Chase** (Cindy/Kendra/Morgan rejected); picker clips regenerated w/ Horus persona greeting ("I am Horus, your AI assistant" — voice names never spoken); `voice.configs` default tts_voice → Rachel (`20260917020000`). Test line: +1 855 585 5518 (resting on Rachel).
- **Schema: ✅ applied (`20260917000000_voice_schema.sql`)** — new `voice` domain schema (one-domain-per-schema pattern): `voice.phone_numbers` (1/org, UNIQUE org+number), `voice.configs` (per-org receptionist config, server CHECKs for transfer/forward numbers), `voice.calls` (per-call records incl. **recording_url** — everything is stored: recordings, transcripts, messages. **Clients** read/listen in their inbox (transcripts via conversations channel 'voice', recordings via customer-api); **employees** get aggregates-only — guard is gateway-level like chat logs: no dashboard-api action returns call rows; RLS org-isolation guards direct roles). `voice.configs` also got `recording_enabled` (default true, 2-party-consent disclosure). **`voice` added to PostgREST exposed schemas** (ALTER ROLE authenticator pgrst.db_schemas + NOTIFY reload — mgmt PATCH alone didn't propagate) so edge functions can write calls and customer-api can read recordings. `core.phone_numbers` + `core.voice_configs` security-invoker bridge views (verified 200 via PostgREST). `conversations.channel` CHECK now includes `'voice'`. RLS org-isolation on all 3. Still TODO in 1a: `core.org_services` voice row per org; verify `crm.contact_aliases` phone aliases.
- **UI shell shipped (`356a3e1`, preview-only)** — third pill in Channels (Website Chat / Email / Voice; demos never see it). Status strip (enabled · assistant sync · minutes→Billing), Phone Number card (Softphone-style provisioning modal: area-code search → debounced mock results w/ capability badges → confirm summary; no prices — bundled; US-only, 1/org; typed-confirm release), AI Receptionist card (enable toggle, 6 real Telnyx Ultra voices w/ actual TTS preview MP3s `492c338` — Cindy/Rachel/Kendra/Reed/Carson/Chase, real catalog IDs), Escalation & Fallback card (warm transfer + number, fallback pill: TTS voicemail greeting / forward calls, after-hours pill driven by Business Details hours), Usage card (aggregates only — privacy rule). All session-local mock state, marked "UI preview". Next: Phase 0 latency spike (needs fresh Telnyx key — `.env` one is stale, working one lives in Softphone `.env`), then Phase 1b backend (dashboard-api actions: get/update voice config, number search/order/release; handle-inbound-call webhook). Plan refreshed at `83cfa38` (UI-first order, privacy rule, bundled pricing, demos excluded, kb_versions refs).
