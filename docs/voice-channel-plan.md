...
> - **Privacy model (hard rule)**: everything is STORED — call records, recordings, transcripts, messages (clients read/listen to them in THEIR dashboard inbox; developers have DB access). The rule is purely about EMPLOYEE access: the employee dashboard shows **aggregate usage only** — calls answered, total minutes, average duration, credits burned. NO caller numbers, NO per-call rows, NO recordings, NO transcripts for employees. Enforced at the gateway layer (same as chat logs): dashboard-api simply has no action that returns `voice.calls` rows — aggregates-only RPC. RLS org-isolation guards direct anon/authenticated access; no `core` bridge view for calls = defense-in-depth only.
...
- ⚠️ **Ultra heteronym stumbles** — "live"→"leave" etc. on Cindy/Kendra/Morgan (rejected via owner call-testing). **Approved shortlist: Rachel, Amber, Reed, Carson, Chase** (2F/3M). Mitigations baked into the prompt template: never use the word "live" (adjective) in generated text; per-org pronunciation dictionary (`voice_settings.pronunciation_dict_id`) planned in Phase 1 for proper nouns (business names, street names).
...
- Grants per schema-reorg pattern; PostgREST reaches voice tables via the `core` bridge views ✓ (verified live: `core.phone_numbers` + `core.voice_configs` return 200) AND directly via `.schema("voice")` — **`voice` added to PostgREST exposed schemas 2026-09-17** (mgmt PATCH didn't propagate to the `authenticator` role setting → set `ALTER ROLE authenticator SET pgrst.db_schemas = '...,voice'` directly + `NOTIFY pgrst, 'reload config'` + `NOTIFY pgrst, 'reload schema'`). Needed anyway: edge functions write call records through PostgREST, and customer-api reads recordings for the client inbox. Employee guard is gateway-level (no dashboard-api action returns call rows), not schema exposure.
...
- ~~`handle-call-events`~~ **FOLDED into `handle-inbound-call`** (2026-09-14): assistant events (`call.conversation.ended` / `call.conversation_insights.generated`) are delivered to the CALL's webhook URL — the Call Control app — so a separate endpoint could never receive them. Transcript → messaging.conversations/messages (channel 'voice', caller → CRM contact via phone alias), recording+cost → voice.calls, credits burn at hangup (whole minutes × org rate via `increment_pool_usage`). TODO follow-up: Telnyx recording download URLs are S3-presigned (~10 min TTL) — archive to Supabase Storage for permanent client access.

**1d. Assistant sync:** ✅ LIVE (2026-09-14, in dashboard-api `sync_voice_assistant` — not a shared module) — builds voice instructions from org profile/hours/services + full active KB (voice wrapper: short spoken answers, no markdown, never "live", timezone-aware `{{telnyx_current_time}}` dynamic var); creates or PATCHes the per-org assistant (Kimi-K2.6, deepgram/flux, dual/mp3 recording, Ultra voice, greeting); hangup tool attached ALWAYS (inline — API-created assistants get no defaults); **transfer tool attached when `transfer_enabled`** (`from` = org's number on the shared app, premium voicemail detection → `stop_transfer` = AI takes the call back, takes a message); GET-verify with one retry (PATCH wiping bug) before stamping `synced_at`/`instructions_version`; manual Sync button on the Voice tab + Synced/Needs-sync badge. ✅ Verified: attach/detach round-trip on HD org (transfer → +16067550460).
