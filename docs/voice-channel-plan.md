...
> - **Privacy model (hard rule)**: employees see **aggregate usage only** — calls answered, total minutes, average duration, credits burned. NO caller numbers, NO per-call rows, NO recordings, NO transcripts anywhere in the employee dashboard. Calls ARE recorded + transcribed (owner decision 2026-09-17) — recordings/transcripts are internal/customer-domain data, stored server-side. Conversations land in the CUSTOMER's inbox (channel `voice`). Employee APIs must not expose `voice.calls` rows (enforced structurally: no `core` bridge view for the table).
